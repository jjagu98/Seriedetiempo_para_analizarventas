...
library(TSA) #proporciona herramientas y funciones para el análisis de series temporales en R.
library(readr)#proporciona funciones eficientes para leer datos en formato rectangular, como CSV o archivos de texto
library(forecast)#ofrece funciones para el análisis y pronóstico de series temporales en R. 
# Cargar el archivo Excel
data <- read_excel("C:/Users/juanjoseagudelo/OneDrive/PRUEBA_TECNICA_PCO/ventas_tecnologia.xlsx")
# Crear una serie de tiempo mensual
ventas_ts <- ts(data$valor_transaccion, start = c(2020, 9), end = c(2022, 8), frequency = 12)
# Visualizar la serie de tiempo y sus funciones de autocorrelación
plot(ventas_ts)  
par(mfrow=c(1,2))
acf(ventas_ts,lag=48)
pacf(ventas_ts,lag=48)

# Función para buscar la mejor combinación de parámetros para SARIMA
...
# Ejecutar la función para buscar los mejores parámetros SARIMA
...
# Análisis de los residuales del modelo ARIMA ajustado
res=residuals(mod)
# Código para la prueba de Ljung-Box
...
# Gráficos de los residuales y pruebas estadísticas
par(mfrow=c(3,2))
...
# Gráfico de la serie original y los valores ajustados
par(mfrow=c(1,1))
